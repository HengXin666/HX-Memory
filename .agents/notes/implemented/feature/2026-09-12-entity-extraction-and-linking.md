...
边建起来之后, 同一批数据上出现两个方向相反的读数 —— 差别只在**指标怎么定义**。

**读数一 (全体 171 个 case): 图扩展看着有害。**

| 指标 | 图关闭 | 图开启 |
| --- | --- | --- |
| 全体 R@1 | **0.690** | 0.313 |
| 全体 R@10 | 0.947 | 0.949 |
| 图候选的 gold 精确率 | — | **8.5%** (50/588) |

但这一批 case 的 gold **全部字面可命中**, 图扩展召回次级目标时无从加分;
它带进来的"主题邻居"却会参与 RRF 排名, 于是只看到代价。

**读数二 (只在字面不可达的 case 上): 图扩展有真实价值。**

新增 `bench/lib/graph-cases.ts` 构造"必须靠边才能答对"的 case: 只在**语义承载型边**
(generalizes/supersedes/sameAs/contradicts/instanceOf) 上生成, 不用 `relates` (实测那是共现噪声);
并且机器校验目标正文与 query 的 bigram 重合 **<= 0.15** —— 否则目标本来就能被词面召回, 不构成图专属 case。

| | 图关闭 | 图开启 |
| --- | --- | --- |
| graph_only 命中 | 1/4 | **3/4** |

命中路径也印证了机制: 图开启后那两个目标是以 `via=graph` 被拿到的, 关闭时完全拿不到。

**注意样本只有 4 条**: 12 条候选边里 9 条被拒绝, 因为两端**字面重合度过高** (0.29~0.75) ——
也就是说当前库里的边大多连接着本来就相似的条目, 这本身就是"边没提供新信息"的证据。
因此读数二只能作方向性参考, 不能说"图扩展已被证明有效"。

## 不改默认值, 以及为什么

把图通道降为"仅兜底"能让全体 R@1 回到 0.690 且不损失 R@10/H@10, 但它会**删掉设计能力**:
conformance 明确要求"从命中的种子出发, 把字面不相关的邻居召回并给出可审计的 why"。
读数二证明这项能力在字面不可达时真的有用。因此本改动**只记录测量, 不动默认行为**。

正确的下一步是**分层**而非开关: 让图候选**排在词面结果之后**, 而不是以同等 RRF 权重参与竞争 ——
这样既保住读数一里的精度, 又保住读数二里的召回。这需要单独实现与测量, 不该在本次顺带做掉。
