...
- **`verify-structure.ts`** 四条约束:
  1. 单文件 ≤ 1400 行 (当前最大 1373; 阈值是"不许更差", 不是"已达标");
...
**把 file-store.ts 一步拆到位 (目标 400 行)。** 那是大重构, 会与当前的功能修复混在一起难以 review; 先用阈值止血 (不许更差), 拆分作为独立任务。
...
代价: ①新增一条跨文件共用的工具函数时, 若它属于"全局口径", 必须登记到 `SINGLE_SOURCE` 清单 (否则闸门报"多处定义"); ②组装根白名单意味着新增宿主入口要改一次脚本; ③`file-store.ts` 已贴着 1400 行上限, 任何新增都会立刻失败 —— 这是**刻意的**, 它把"该拆了"变成一条会拦人的规则而不是一句建议。
