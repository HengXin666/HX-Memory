...
# 已发布版本 (把 vX.Y.Z 换成 release 里的实际版本): 无需克隆、无需构建
...
> 装完必须**重启 DSH**: 插件层在启动时组装。可用 `dsh --profile web --dump-config`
> 核对输出里有没有 `# == @hengxin666/hx-memory` 段。
>
> 为什么不用 GitHub 的源码 zip 归档: `dsh plugin` 的 URL 依赖走 pnpm 的 tarball 通道, `.zip` 会报
> `ERR_PNPM_TARBALL_EXTRACT`; 而且本仓库的 `dist/` 不进 git, 源码归档即使装上也没有可加载的产物。
> 各安装通道的实测对照见 [docs/plugin-install.md](docs/plugin-install.md)。
...
装好后, 你会在 **DSH 设置区看到三个入口**:

| 入口             | 在哪                 | 用来干什么                                 |
| ---------------- | -------------------- | ------------------------------------------ |
| **记忆审阅**     | DSH 设置 → 记忆审阅  | 攒了一批推广提议时, 逐条**确认/驳回**      |
| **记忆绑定**     | DSH 设置 → 记忆绑定  | 为每个项目声明"要绑哪些记忆源"(VCP 式拓扑) |
| **记忆注入**     | DSH 设置 → 插件      | 选注入时机: **只在首轮一次** / 逐轮补差量  |

> **注入时机**默认为"每轮注入 (差量)": 每轮只补**本会话上下文里还没有的**条目
> (同一批常驻记忆不会重复出现)。不想让它在会话中途自动出现, 就在 **DSH 设置 → 插件 →
> HX-Memory** 里切成"只在首轮注入"; 两种模式下 `memory_search` 工具都常开, 模型仍可按需检索。
...
| ②b  | **注入时机开关**        | DSH 设置 → 插件 → HX-Memory            | 选「只在首轮注入」或「每轮注入 (差量)」                              | 改完**当轮**生效 (不重启): 前者只注入一次, 后者逐轮补新条目     |
